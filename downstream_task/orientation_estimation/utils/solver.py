...
class Solver(gorilla.solver.BaseSolver):
    def __init__(self, model, loss, dataloaders, logger, cfg,check_point_prefix=None):
        ...
        if cfg.checkpoint_epoch != -1:
            ...
            checkpoint = os.path.join(cfg.log_dir, f'{check_point_prefix}_epoch_' + str(cfg.checkpoint_epoch) + '.pth')
    ...
    def solve(self,check_point_prefix):
        while self.epoch <= self.cfg.max_epoch:
            ...
            ckpt_path = os.path.join(
                self.cfg.log_dir, f'{check_point_prefix}_epoch_{self.epoch}.pth')
            ...
            self.epoch += 1
            # 因VINET训练时间长了会有bug产生导致训练进程中断，本处为了使网络训练自动重启，使用break跳出循环,每训练完成一个epoch后重启训练脚本，保证训练可以继续进行
            print("Breaking after one epoch to allow external restart for robustness.")
            break
...
def test_r_func(r_model, dataloder, save_path):
    r_model.eval()
    with tqdm(total=len(dataloder)) as t:
        for i, data in enumerate(dataloder):
            path = dataloder.dataset.result_pkl_list[i]
